...
# (now uses min_ts/max_ts in the API call directly)
# -------------------------
def fetch_settlements_last_n_days(client, days=1):
    ...
    while True:
        ...
        if getattr(resp, "settlements", None):
...
# -------------------------
# Compute portfolio / P&L stats from fills + settlements
# -------------------------
...
# -------------------------
def generate_summary_json(days=365):
    client = load_kalshi_client()

    # 1) True account-level numbers from Kalshi (matches app)
    ...
    # 2) Trading activity over last N days
    ...
    summary = {
        ...
        "lookback_days": days,
        "account": account,          # <- matches Kalshi portfolio screen
        "fills_last_n_days": fills_dict,
        "settlements_last_n_days": settlements_dict,
        "summary": stats,            # <- investment / reinvestment / P&L
    }
